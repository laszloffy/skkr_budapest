...
c     (rel):    mdim=mdimr,  lkmax=kmymax=2*lkmax
c------------------------------------------------
...
c
      ...
      complex*16 rg2d(kmymaxp,kmymaxp)
      complex*16 m00(mdim,mdim),m01(mdim,mdim),m10(mdim,mdim)
c
      complex*16 fac ! added 01/03/2021 laszloffy
      common/relfac/fac
c
...
c
        do j=jmin,-i
          jp=nim+i+j
          kkj = (jp-1)*lkmax
          if(irel.eq.1) call relmtrx(g2d(1,1,j,i),rg2d,fac,lmax)
          do k1=1,lkmax
            kk1 = k1+kki
            do k2=1,lkmax
              kk2 = k2+kkj
