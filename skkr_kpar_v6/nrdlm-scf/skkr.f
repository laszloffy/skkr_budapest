...
c
      logical rel,bulk,dos,bulkgeo,nspheres
...
c
      dimension dosa(0:lmaxp,mintfc,2,me),dosb(0:lmaxp,mintfc,2,me)
      dimension doslma(lmmaxp,mintfc,2,me),doslmb(lmmaxp,mintfc,2,me)
...
c
      open(unit=5,file='input_nrdlm.in',status='old')
c     ------------------------------------------------------------------
     ...
     >bulk,rel,dos,nspheres,rightm,newvac,ivacpot,v0,v00,vrsh,
     >iscreen,vscreen,itscfmax,tolerr,tolef,intbz,kunning)
c     ------------------------------------------------------------------
      close(5)
c
...
c
      ...
      if(itest.ge.2.or.kset(ie).eq.0) then
        write(6,'('' kset for BZ integration:'',i3)') kset(ie)
...
c     ------------------------------------------------------------------
      call cpacoord(
     > itscf,ie,ce,we(ie),lmax,nintfc,eta,rightm,
     > iesublatt,bulk,bulkgeo,kset(ie),xk,wk,nk,intbz,iek,
...
c     ------------------------------------------------------------------
     ...
     > dosa(0,1,1,ie),doslma(1,1,1,ie),qwpa,qwa,enbwa,qmomwa,rhowa,
     > dosb(0,1,1,ie),doslmb(1,1,1,ie),qwpb,qwb,enbwb,qmomwb,rhowb)
c     ------------------------------------------------------------------
c
 10   continue
c
...
c
      do li=1,nintfc
        ...
        qmagva(li)=qwa(li,1)-qwa(li,2)
        qmagvb(li)=qwb(li,1)-qwb(li,2)
        do l=0,lmax
          qvpa(l,li)=qwpa(l,li,1)+qwpa(l,li,2)
          qvpb(l,li)=qwpb(l,li,1)+qwpb(l,li,2)
          qmagvpa(l,li)=qwpa(l,li,1)-qwpa(l,li,2)
          qmagvpb(l,li)=qwpb(l,li,1)-qwpb(l,li,2)
          dosla(l,li,ne)=dosa(l,li,1,ne)+dosa(l,li,2,ne)
          doslb(l,li,ne)=dosb(l,li,1,ne)+dosb(l,li,2,ne)
        end do
        do i=1,lmmaxs
        ...
        do i=1,ns(li)
          ...
          rhomagva(i,li)=rhowa(i,li,1)-rhowa(i,li,2)
          rhomagvb(i,li)=rhowb(i,li,1)-rhowb(i,li,2)
        end do
      end do
c
...
c     ------------------------------------------------------------------
     ...
     >itscfcont,lmax,nintfc,ne,cear,efermi,conc,bulk,
