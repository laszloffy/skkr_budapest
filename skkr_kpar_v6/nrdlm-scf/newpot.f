...
c
c moments of charge densities for Madelung potential
c
      qif=0.d0
      do li=1,nintfc
        ...
        if(itest.ge.2)
     >  write(6,'('' I'',i4,''  Layer'',i2,''  q_A ='',f15.10,
     >            ''  q_B ='',f15.10,''   q='',f15.10)') 
     >            itscf,li,qa,qb,qlay(li)
        qif=qif+qlay(li)
      end do
      write(6,'('' I'',i4,''  Interf.  q='',f15.10)') itscf,qif
c       
      if(.not.bulk) then
        do li=1,ninprc(0)
          do i=1,lmmaxs
...
c smooth charge density near to origo (for vacuum layers useful)
        ...
        do i=n1,1,-1
          ...
          r=dexp(x)
          fpirr=fpi*r*r
          den=rhova(i,li)/fpirr
          if(den.gt.0.d0) then
            den0a=den
          else
            den=den0a
          end if
          rhova(i,li)=fpirr*den
          den=rhovb(i,li)/fpirr
          if(den.gt.0.d0) then
            den0b=den
          else
            den=den0b
          end if
          rhovb(i,li)=fpirr*den
        end do
c
        x=x0
        do i=1,n1
...
c potential energy (part of kinetic energy)
        ...
        do i=1,n1
          rx(i)=rhomaga(i,li)*oldbxca(i)
        end do
        enmaga(li)=-rsimp(rx,rgrid,n1,dx1)
        do i=1,n1
          rx(i)=rhomagb(i,li)*oldbxcb(i)
        end do
...
c
        do i=1,n1
          ...
          rhup(i)=0.5d0*(rhota(i,li)-rhomaga(i,li))/fpir2
          rhdn(i)=0.5d0*(rhota(i,li)+rhomaga(i,li))/fpir2
          drdi(i)=rgrid(i)*dx1
        end do
c
c       if(ixch.ge.10) then
...
c
        if(ixchout.ge.1.and.li.le.3) then
          if(li.eq.1) xcout='xcout.Al'
          if(li.eq.2) xcout='xcout.V'
          if(li.eq.3) xcout='xcout.Mn'
          open(20,file=xcout,status='unknown') 
          do i=1,n1
          write(20,'(13d15.6)') rgrid(i),
     1    rhup(i),drhup(i),d2rhup(i),xpot1(i),
     2    rhdn(i),drhdn(i),d2rhdn(i),xpot2(i)
          enddo
          close(20)
        end if
        if(itest.ge.3) then
...
c
        do i=1,n1
          ...
          rhup(i)=0.5d0*(rhotb(i,li)-rhomagb(i,li))/fpir2
          rhdn(i)=0.5d0*(rhotb(i,li)+rhomagb(i,li))/fpir2
          drdi(i)=rgrid(i)*dx1
        end do
c
c       if(ixch.ge.10) then
...
c
        if(itest.ge.2.and.li.eq.1) then
          ...
          write(6,'(4d20.12)') (bra(i,li),i=1,n1)
          write(6,*)
c         write(6,'('' Total potential'')')
c         write(6,'(4d20.12)') (vrb(i,li),i=1,n1)
c         write(6,'('' magnetic field'')')
c         write(6,'(4d20.12)') (brb(i,li),i=1,n1)
        end if
c
      end do
