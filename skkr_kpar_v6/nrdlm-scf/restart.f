c     Name of Copyright owners: Gabor Csire, Andras Laszloffy, Bendeguz Nyari, Laszlo Szunyogh, and Balazs Ujfalussy
      subroutine restart_in(itscf0,emax,v0,conc,vra,vrb,bra,brb,
...
c
      include '../param.h'
c
      logical rest,bulk,cpalay
      character*30 update,printout,laycore
...
c
      dvc=137.036d0
      ij=index(printout,' ')-5
      update=printout(1:ij)//'.update'
c
      inquire(file=update,EXIST=rest)
      if(rest) then
         open(12,file=update,status='OLD',form='UNFORMATTED')
         read(12) relnum
         read(12) itscf0
         write(6,'(/'' RESTARTING from iteration '',i4, ''!!!''/)') 
     >   itscf0
         if(bulk) then
            emax=relnum
         else
...
c
      subroutine restart_out(itscf,emax,v0,conc,vra,vrb,bra,brb,
...
c
      ...
      update=printout(1:ij)//'.update'
c
      open(12,file=update,status='UNKNOWN',form='UNFORMATTED')
      rewind(12)
c
      if(bulk) then
         relnum=emax
