...
c
     ...
     &    btha,bpha,bthb,bphb,jspd1,jspd2,sxa,sxb,iesubl(il)
          if(jspd1.ne.1) jspd1=-1
          if(jspd2.ne.1) jspd2=-1
c
          do i=1,3
...
c
      do iprc=1,nprc+1
        if(ninprc(iprc).ne.ninprc(iprc-1)) then
          write(6,'('' Size of PL mismatch'',2i3,3x,2i3)') 
     >    iprc-1,ninprc(iprc-1),iprc,ninprc(iprc)
          goto 100
        end if
      end do
      ninprc0=ninprc(0)
      ...
      do iprc=1,nprc+1+2*nextra
        do inprc=1,ninprc0
          il=il+1
          do i=1,3
            if(dabs(cvec(il+ninprc0,i)-cvec(il,i)-cshift(i)).
     >      gt.ctol) then
             write(6,'('' Lattice vector mismatch'',2i3)') 
     >       il+ninprc0,il
             write(6,'(3d18.8)') cvec(il+ninprc0,i),cvec(il,i)+cshift(i)
             goto 100
            end if 
          end do
        end do
