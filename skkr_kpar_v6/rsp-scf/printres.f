...
c
      do li=1,nintfc
        cpalay=1.d0-conc(li).gt.tiny
        write(7,'(''##'',i3,t10,''DOS   A'')') li
        write(8,'(''##'',i3,t10,a)') li,idpota(li)
c       ---------------------------------------
        call dosprint(lms,lmax,li,ne,cear,dosa)
c       ---------------------------------------
        if(cpalay) then
          write(7,'(''##'',i3,t10,''DOS   B'')') li
          write(8,'(''##'',i3,t10,a)') li,idpotb(li)
c         ---------------------------------------
...
c
      write(7,'(/29x,''Spin magnetic moments :'',
     >          /29x,''----------------------''/ 
     >          /29x,''         X            '')') 
c     ------------------------------------------------
...
c     ------------------------------------------------
c
      write(7,'(/29x,''Orbital magnetic moments :'',
     >          /29x,''----------------------''/ 
     >          /29x,''         X            '')') 
c     ------------------------------------------------
      call printmag(nintfc,lmax,conc,
...
c
      write(7,'(//''Moments of charge densities:''
     >           /''---------------------------''/)')
      i=0
      do l=0,2*lmax 
      do m=-l,l     
...
c
      ...
      dimension ddd(kmaxp),ppp(0:lmaxp,2,me)
c
      real*8 sume(me)
      complex*16 cear(me)
c
...
c
      do ie=1,ne
        ...
        if(.not.lms) then
          ...
          write(7,'(f10.6,2x,10f10.5)') er,(ddd(k),k=1,kmax),sum
        else
          if(ie.eq.1) write(7,'(''SPIN 1 '')')
          if(ie.eq.1) write(8,'(''SPIN 1 '')')
          ii=0
          do is=1,2
             sume(ie)=0.d0
             do l=0,lmax
                ...
                do m=-l,l
                   ii=ii+1
                   sume(ie)=sume(ie)+dos(ii,li,ie)
                   ppp(l,is,ie)=ppp(l,is,ie)+dos(ii,li,ie)
                end do
             end do
c             write(7,'(f12.8,2x,10f10.5)') er,(ppp(l,is),l=0,lmax),sum
               if(is.eq.1) then
                  write(7,'(f12.8,6e15.8)')
     >             er,(ppp(l,1,ie),l=0,lmax),sume(ie)
                  write(8,'(f12.8,10e17.8)')
     >            er,(dos(l,li,ie),l=1,lmmax),sume(ie)
               end if
          end do
        end if
      end do
c
      if(lms) then
        ...
        do ie=1,ne
          er=dreal(cear(ie))
          write(7,'(f12.8,6e15.8)') er,(ppp(l,2,ie),l=0,lmax),sume(ie)
          write(8,'(f12.8,10e17.8)')
     >   er,(dos(l,li,ie),l=lmmaxp+1,lmmaxp+lmmax),sume(ie)
...
c
      ...
      do k=1,kmax
         ...
         nk=2*iabs(kap)
         kmy1=kmy0+nk-1
         write(7,'(t10,10f11.6)') (qvp(kmy),kmy=kmy0,kmy1)
         kmy0=kmy1+1
      end do
c
      return
...
c
      if(.not.lms) then
        ...
        write(7,'('' Layer'',i2,2x,a1,2x,t15,10f11.6)') li,
     >              ab,(ddd(k),k=1,kmax),qv(li)
        else
        write(7,'(''      '',4x,a1,2x,t15,10f11.6)')
     >              ab,(ddd(k),k=1,kmax),qv(li)
        end if
      else
        sum=0.d0
