...
c
      include '../param.h'
c
      logical wrel,lms,cpalay,linbw,dos
c
...
c        
     ...
     >                  ddphenb,ddthenb,d2dphenb,d2dthenb,d2dthphenb)
         ddphenba(li)=ddphenba(li)+ddphenb
         ddthenba(li)=ddthenba(li)+ddthenb
         d2dphenba(li)=d2dphenba(li)+d2dphenb
         d2dthenba(li)=d2dthenba(li)+d2dthenb
         d2dthphenba(li)=d2dthphenba(li)+d2dthphenb
c
       if(dos.and.(itest.ge.2)) write(6,'(''DEDTH'',2i4,2f10.6,d20.10)')
     >   li,ie,ce,ddthenb
c
         do irad=1,ns(li)
...
c        if(li.eq.8) write(6,'(''Energy'',i5,''Layer'',i5)') ie,li
         do i=1,lmmaxs
           do j=1,lmmaxs
             qmoma(i,li)=qmoma(i,li)+qmom(j)*rmatpa(j,i,li)
           end do
c          if(li.eq.8) write(6,'(''Q'',i2,2x,2d13.5)') i,qmoma(i,li)
         end do
c
c Check charges
...
c        
     ...
     >                  ddphenb,ddthenb,d2dphenb,d2dthenb,d2dthphenb)
         ddphenbb(li)=ddphenbb(li)+ddphenb
         ddthenbb(li)=ddthenbb(li)+ddthenb
         d2dphenbb(li)=d2dphenbb(li)+d2dphenb
         d2dthenbb(li)=d2dthenbb(li)+d2dthenb
         d2dthphenbb(li)=d2dthphenbb(li)+d2dthphenb
c
         do irad=1,ns(li)
