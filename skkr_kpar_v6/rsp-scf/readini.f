...
c
      logical bulk,dos,nspheres,wrel,lms,cpatest,cpain
...
c
      read(5,*) qc
      write(6,'(/'' Charge of external capacitor:'',f8.4)') qc
c
      if(qc.gt.1.0d-12) then
       write(6,'('' Position of external capacitor is fixed
     >in the last vacuum layer'')')
      end if
