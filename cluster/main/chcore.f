...
c
      integer mmax
      parameter (mmax=mintfc)
c
      logical cpalay
      character*30 for006
      character*10 idpota(mmax),idpotb(mmax)
      character*3  idel
      character*4  titrea(iorb,mmax),titreb(iorb,mmax)
      character*4  oplus(4),ominus(3)
c
      dimension conc(mmax)
      dimension za(mmax),zb(mmax)
      dimension qca(mmax),qcb(mmax)
      dimension enca(mmax),encb(mmax)
      dimension vra(nrad,mmax),vrb(nrad,mmax)
      dimension rhoca(nrad,mmax),rhocb(nrad,mmax)
      dimension dx(mmax),ns(mmax),rs(mmax)
c
      dimension norba(mmax),norbb(mmax)
      dimension dena(iorb,mmax),denb(iorb,mmax)
      dimension nela(iorb,mmax),nelb(iorb,mmax)
      dimension nqna(iorb,mmax),nqnb(iorb,mmax)
      dimension nka(iorb,mmax),nkb(iorb,mmax)
      dimension nqla(iorb,mmax),nqlb(iorb,mmax)
      dimension dfla(iorb,mmax),dflb(iorb,mmax)
      dimension dq1a(iorb,mmax),dq1b(iorb,mmax)
c
