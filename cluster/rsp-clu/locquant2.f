...
c
     ...
     > vmadid,vmadiq,
     > enba,enbdiffa,denba,nenba,enorba,qmoma,
     > rhova,rhospa,rhodspa,rhomaga,
     > dosmaga,spin_magvpa,spin_magva,orb_magvpa,orb_magva,
c
     > lliter,efermi,enbifcint,qvifcint,omifcint, iwtau, nwtau,
     > enpba)
c
...
c
      ...
      real*8 qva(mmax)
      real*8 qvaorb(kmymaxp,mmax)
      complex*16 nqva(mmax,me)
...
c
     ...
     >                           dimag(we*zdmor(k,i))
            ...
            enpba(k,li)=enpba(k,li)+dimag(we*ce*zdos(k))
            nenba(li,ie)=nenba(li,ie)+ce*zdos(k)
         end do
