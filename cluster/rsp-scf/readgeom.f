...
c       Optional input for a linear Bloch Wall 
        read(5,*) linbw,phi1,phi2,lay1,lay2,shbw
c
...
c
      ...
      do iprc=1,nprc+1+2*nextra
        do inprc=1,ninprc0
          ...
          do i=1,3
            if(i.eq.2) then
              write(6,*) 'bulkgeo test',il+2,i
              write(6,*) dabs(cvec(il+3,i)-cvec(il+2,i))
            end if
            if(dabs(cvec(il+ninprc0,i)-cvec(il,i)-cshift(i)).gt.ctol) 
     >        then 
              write(6,*) 'bulkgeo fails in il=',il
              write(6,*) 'bulkgeo fails in i=',i
              goto 100
            end if
          end do
        end do
