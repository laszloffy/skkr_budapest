...
c         -----------------------------------------
          call gstore(park,psq,eta,bulkgeo,lmax,nintfc,gg2d) ! TODO : ce
c changed to psq
c         -----------------------------------------
...
c
          ...
          if(ichk.gt.0) then
            write(6,'('' Point group'',i3,'' K-point'',i3,2f11.5)')
     >         ig,ik,park(1),park(2)
          endif
c         ------------------------------------------------
...
c         ------------------------------------------------
           call getsfg(yy,ysave(1,1,ikuse,igordl(igel)),
     >                 ndiml,mdim,iwrite)
          else
c           ------------------------
            iread=iwrite
            if(iwrite.eq.1) iread=-1
c           ------------------------
            call getsfg(ysave(1,1,ikuse,igordl(igel)),yy,
     >                  ndiml,mdim,iread)
          endif
...
c         ------------------------------------------------
           call getsfg(xx,xsave(1,1,ikuse,igordr(iger)),
     >                 ndimr,mdim,iwrite)
          else
c           ------------------------
            iread=iwrite
            if(iwrite.eq.1) iread=-1
c           ------------------------
            call getsfg(xsave(1,1,ikuse,igordr(iger)),xx,
     >                  ndimr,mdim,iread)
          endif
c
          if(ige.eq.ig) then
...
c
         if(itest.gt.2) then
            write(6,'(/'' tau -A '',i2)') li
            call replms(taudiag,taua(1,1,li),lmax)
            do k=1,kmymax
              write(6,'(i3,2d20.10)') k,taudiag(k)
            end do
c           call outmat1(taua(1,1,li),kmymax,kmymax,kmymaxp,tol,6)
         end if
c
         if(cpalay) then
c+------------+
...
c rotate tau-matrix to local frame of reference
c
         call repl(gtaub(1,1,li),taua(1,1,li),kmymax,kmymaxp)
c        ---------------------------------------------------
