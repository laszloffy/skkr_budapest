...
c
     > dosa,dosha,doseh,doshe,dosteh,dosthe,qvpa,qva,
     > qvha,qveha,qvhea,qvteha,qvthea,
...
c
      ...
      dimension dosha(kmymaxp,mintfc)
      dimension doseh(kmymaxp,mintfc),dosteh(kmymaxp,mintfc)
      dimension doshe(kmymaxp,mintfc),dosthe(kmymaxp,mintfc)
      dimension dosmaga(kmymaxp,mintfc),dosmagb(kmymaxp,mintfc)
      dimension dosmagh(kmymaxp,mintfc)
      dimension qva(mintfc),qvb(mintfc)
      dimension qvpa(kmymaxp,mintfc),qvpb(kmymaxp,mintfc)
      dimension qvha(mintfc)
      dimension qvhpa(kmymaxp,mintfc)
...
c
      complex*16 qveh,qvhe,qvteh,qvthe
c
      complex*16 rhoveh(nrad)
      complex*16 rhovhe(nrad)
...
c Magnetic moments
        sig=-1.d0
c       ------------------------------------------------- e-e 
        call moment(lmax,lms,sxcoeff,sxbcoeff,sig,
     >              ggrmat(1:kmymax,1:kmymax,0),
     >              fgrmat(1:kmymax,1:kmymax,0),zdmag(1:kmymax,1),rtmp)
        call moment(lmax,lms,sycoeff,sybcoeff,sig,
     >              ggrmat(1:kmymax,1:kmymax,0),
     >              fgrmat(1:kmymax,1:kmymax,0),zdmag(1:kmymax,2),rtmp)
        call moment(lmax,lms,szcoeff,szbcoeff,sig,
     >              ggrmat(1:kmymax,1:kmymax,0),
     >              fgrmat(1:kmymax,1:kmymax,0),zdmag(1:kmymax,3),rtmp)
        call moment(lmax,.true.,lxcoeff,lxbcoeff,sig,
     >              ggrmat(1:kmymax,1:kmymax,0),
     >              fgrmat(1:kmymax,1:kmymax,0),zdmor(1:kmymax,1),rtmp)
        call moment(lmax,.true.,lycoeff,lybcoeff,sig,
     >              ggrmat(1:kmymax,1:kmymax,0),
     >              fgrmat(1:kmymax,1:kmymax,0),zdmor(1:kmymax,2),rtmp)
        call moment(lmax,.true.,lzcoeff,lzbcoeff,sig,
     >              ggrmat(1:kmymax,1:kmymax,0),
     >              fgrmat(1:kmymax,1:kmymax,0),zdmor(1:kmymax,3),rtmp)
c        ------------------------------------------------- h-h
        call moment(lmax,lms,sxcoeff,sxbcoeff,sig,
     >              ggrmat(kmymax+1:2*kmymax,kmymax+1:2*kmymax,0),
     >              fgrmat(kmymax+1:2*kmymax,kmymax+1:2*kmymax,0),
     >              zdmag(kmymax+1:2*kmymax,1),rtmp)
        call moment(lmax,lms,sycoeff,sybcoeff,sig,
     >              ggrmat(kmymax+1:2*kmymax,kmymax+1:2*kmymax,0),
     >              fgrmat(kmymax+1:2*kmymax,kmymax+1:2*kmymax,0),
     >              zdmag(kmymax+1:2*kmymax,2),rtmp)
        call moment(lmax,lms,szcoeff,szbcoeff,sig,
     >              ggrmat(kmymax+1:2*kmymax,kmymax+1:2*kmymax,0),
     >              fgrmat(kmymax+1:2*kmymax,kmymax+1:2*kmymax,0),
     >              zdmag(kmymax+1:2*kmymax,3),rtmp)
        call moment(lmax,.true.,lxcoeff,lxbcoeff,sig,
     >              ggrmat(kmymax+1:2*kmymax,kmymax+1:2*kmymax,0),
     >              fgrmat(kmymax+1:2*kmymax,kmymax+1:2*kmymax,0),
     >              zdmor(kmymax+1:2*kmymax,1),rtmp)
        call moment(lmax,.true.,lycoeff,lybcoeff,sig,
     >              ggrmat(kmymax+1:2*kmymax,kmymax+1:2*kmymax,0),
     >              fgrmat(kmymax+1:2*kmymax,kmymax+1:2*kmymax,0),
     >              zdmor(kmymax+1:2*kmymax,2),rtmp)
        call moment(lmax,.true.,lzcoeff,lzbcoeff,sig,
     >              ggrmat(kmymax+1:2*kmymax,kmymax+1:2*kmymax,0),
     >              fgrmat(kmymax+1:2*kmymax,kmymax+1:2*kmymax,0),
     >              zdmor(kmymax+1:2*kmymax,3),rtmp)
c        -------------------------------------------------
c       call cpu_time(tfinish)
...
c
        do k=1,kmymax
          ...
          doseh(k,li)=dimag(zscdos(k))               ! electron-hole part 
          doshe(k,li)=dimag(zscdos(k+kmymax))        ! triplet part 
          dosteh(k,li)=dimag(zsctdos(k))               ! triplet up part 
          dosthe(k,li)=dimag(zsctdos(k+kmymax))        ! triplet down part 
          dosmaga(k,li)=dimag(rtmp(1)*zdmag(k,1)     ! electron part of magnetic dos
     >                       +rtmp(2)*zdmag(k,2)
     >                       +rtmp(3)*zdmag(k,3))  ! rtmp either rz or rba
          dosmagh(k,li)=dimag(rtmph(1)*zdmag(k+kmymax,1)    ! hole part of magnetic dos
