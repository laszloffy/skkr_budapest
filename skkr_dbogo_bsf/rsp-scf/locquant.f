...
c
        tautest=.false.
        if(tautest) then
        write(6,*) " <locquant> : zdos(1), zdos(10) ",zdos(1),zdos(10)
        write(6,*) " <locquant> : zqmom(1,1), zqmom(10,1) ",
     >    zqmom(1,1),zqmom(10,1)
        end if
c
...
c
        tautest=.false.
        if(tautest) write(6,*) '<locquant> : tautest dosa li=',li
        do k=1,kmymax
...
c           write(6,'(''Q    '',d13.5)') qva(li)
           do i=1,lmmaxs
             write(6,'(''Qmom - electron '',i2,2x,2d13.5)') 
     >             i,qmoma(i,li)
             write(6,'(''Qmom - hole     '',i2,2x,2d13.5)') 
     >             i,qmomha(i,li)
           end do
...
c
           ...
           do i=1,3
c            spin_magvpb(k,li,i)=spin_magvpa(k,li,i) ! laszloffy commented on 2020/09/04
c            spin_magvhpb(k,li,i)=spin_magvhpa(k,li,i) ! laszloffy commented on 2020/09/04
             spin_magvb(li,i)=spin_magva(li,i)
             spin_magvhb(li,i)=spin_magvha(li,i)
c            orb_magvpb(k,li,i)=orb_magvpa(k,li,i) ! laszloffy commented on 2020/09/04
c            orb_magvhpb(k,li,i)=orb_magvhpa(k,li,i) ! laszloffy commented on 2020/09/04
