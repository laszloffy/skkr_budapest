...
c     ----------------------------------------------------------------
      ...
      if(orbpol) then
        call sublattpot(nintfc,ns,gbopra,iesublatt)
        call sublattpot(nintfc,ns,gboprb,iesublatt)
      end if
c
...
c
      subroutine deltamix(itscf,itscfcont,nintfc,lmax,iesublatt,
     > dx,ns,rs,ferr1,rlambda,delta,qveha,qvhea)
c
      implicit real*8 (a-h,o-z)
c
      include '../param.h'
c
      parameter (mbig=2*nrad*mintfc+2)
c
      logical bulk,cpalay,orbpol,opot
      character*1 rightm
c
...
c
      dimension redelta(nrad,mintfc)
      dimension cimdelta(nrad,mintfc)
c
      complex*16 delta(nrad,mintfc)
      complex*16 gdelta(nrad,mintfc) 
c
...
c
      ...
      do li=1,nintfc
      do i=1,ns(li)
      redelta(i,li)=rlambda(li)*dreal(qveha(li))
      cimdelta(i,li)=rlambda(li)*dimag(qveha(li))
      gdelta(i,li)=CMPLX(redelta(i,li),cimdelta(i,li))
      end do
      end do
c
c     TODO: gdelta is complex, not real. call commented by Laszloffy
c 07/07/2021
c
c      call sublattpot(nintfc,ns,gdelta,iesublatt)
c
c - Potential mixing
c
      ibig=0
      do li=1,nintfc
        ...
        do i=1,ns(li)
          r=dexp(x)
          ibig=ibig+1
          xbr(ibig)=dreal(delta(i,li))
          fbr(ibig)=dreal(gdelta(i,li))-dreal(delta(i,li))
          ibig=ibig+1
          xbr(ibig)=dimag(delta(i,li))
          fbr(ibig)=dimag(gdelta(i,li))-dimag(delta(i,li))
          x=x+dx(li)
        end do
      end do
c
...
c
      ...
      call dott(fbr,fbr,t1,nbig1)
      ferr1=dsqrt(t1/err1)
      write(6,'(/'' Error for Delta(r) :'',3d17.6)')  t1,err1,ferr1
c
      if(itscf.eq.1) then
        ibrcase=1
        ibrmax=nbrmax(1)
...
c
      ...
      do li=1,nintfc
        ...
        do i=1,ns(li)
          r=dexp(x)
          ibig=ibig+1
          redelta(i,li)=xbr(ibig)
          ibig=ibig+1
          cimdelta(i,li)=xbr(ibig)
          delta(i,li)=CMPLX(redelta(i,li),cimdelta(i,li))
          x=x+dx(li)
        end do
      end do
c
