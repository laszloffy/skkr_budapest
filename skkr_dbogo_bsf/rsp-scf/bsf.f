...
c we need tauinv and D matrices only for cpa
        ...
        call cpu_time(tfinish)
c        write(6,'(''   Time ellapsed in gjinv2:'',t35,f8.1,'' s'')')
c     >  tfinish-tstart
        call cpu_time(tstart)
...
c        call outmat1(dmatp(:,:,2),2*kmymax,2*kmymax,dbogomaxp,tol,6)
        call cpu_time(tfinish)
c        write(6,'(''   Time ellapsed with dmats:'',t35,f8.1,'' s'')')
c     >  tfinish-tstart
        end if 
c
c========================================
...
c         write(6,*) 'we have single sum contribution for', icpa
            ggrmat_tmp = (0.d0,0.d0)
            fgrmat_tmp = (0.d0,0.d0)
...
c           write(6,*) 'After gf for A0'
       ggrmat(1:2*kmymax,1:2*kmymax,0) = ggrmat(1:2*kmymax,1:2*kmymax,0)
     >        +cab(icpa) * ggrmat_tmp(1:2*kmymax,1:2*kmymax,0)
       fgrmat(1:2*kmymax,1:2*kmymax,0) = fgrmat(1:2*kmymax,1:2*kmymax,0)
     >        +cab(icpa) * fgrmat_tmp(1:2*kmymax,1:2*kmymax,0)
c            call addmatc(ggrmat(:,:,0),ggrmat_tmp(:,:,0),cab(icpa),
c     >           2*kmymax,dbogomaxp)
c            call addmatc(fgrmat(:,:,0),fgrmat_tmp(:,:,0),cab(icpa),
c     >           2*kmymax,dbogomaxp)
            call cpu_time(tfinish)
c            write(6,'(''   Time ellapsed in A0:'',t35,f8.1,'' s'')')
c     >      tfinish-tstart
...
c
       ggrmat(1:2*kmymax,1:2*kmymax,0) = ggrmat(1:2*kmymax,1:2*kmymax,0)
     >        +cij * ggrmat_tmp(1:2*kmymax,1:2*kmymax,0)
       fgrmat(1:2*kmymax,1:2*kmymax,0) = fgrmat(1:2*kmymax,1:2*kmymax,0)
     >        +cij * fgrmat_tmp(1:2*kmymax,1:2*kmymax,0)
c          call addmatc(ggrmat(:,:,0),ggrmat_tmp(:,:,0),cij,
c     >         2*kmymax,dbogomaxp)
c          call addmatc(fgrmat(:,:,0),fgrmat_tmp(:,:,0),cij,
c     >         2*kmymax,dbogomaxp)
          call cpu_time(tfinish)
c          write(6,'(''   Time ellapsed in A1:'',t35,f8.1,'' s'')')
c     >    tfinish-tstart
c         end if
       end do ! jcpa
       end do ! icpa
