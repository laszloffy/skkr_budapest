...
c
c -read cluster geo file, if needed
      if((.not.clucalc).and.hostprint) then
...
c -allocate cluster t and tau matrices
     ...
     >                  STAT = AllocateStatus)
        ...
        ALLOCATE ( tau_ij1(nbogomax,nbogomax,npair), ! only for one energy point
     >                  STAT = AllocateStatus)
        call alloccheck( AllocateStatus,'tau_ij in skkr' )
        ALLOCATE ( tau_ji1(nbogomax,nbogomax,npair), ! only for one energy point
     >                  STAT = AllocateStatus)
        call alloccheck( AllocateStatus,'tau_ji in skkr' )
      end if
      if(clucalc) then
        ALLOCATE ( tminvh(nbogomax,nbogomax,nintfc,net),
     >                  STAT = AllocateStatus)
        call alloccheck( AllocateStatus,'tminvh in skkr' )
        ALLOCATE ( taucl(nbogomax,nbogomax,nimp),
     >                  STAT = AllocateStatus)
        call alloccheck( AllocateStatus,'taucl in skkr' )
        ALLOCATE ( gtaucl(nbogomax,nbogomax,nimp),
     >                  STAT = AllocateStatus)
        call alloccheck( AllocateStatus,'gtaucl in skkr' )
      end if
c
c -read Left,Right and Layer potentials
c
c     ------------------------------------------------------------------
      if(.not.clucalc) then ! layer case
        call pothandle(
...
c     - - - - - - - - - - - - - - - - - - - - - -
c     bnyari:  Allocate impurity potentials
        ALLOCATE ( vrimp(nrad,nimp), STAT = AllocateStatus)
        call alloccheck( AllocateStatus,'vrimp in skkr' )
        ALLOCATE ( brimp(nrad,nimp), STAT = AllocateStatus)
        call alloccheck( AllocateStatus,'brimp in skkr' )
        ALLOCATE ( boprimp(nrad,2,nimp), STAT = AllocateStatus)
        call alloccheck( AllocateStatus,'boprimp in skkr' )
        ALLOCATE ( deltaimp(nrad,nimp), STAT = AllocateStatus)
        call alloccheck( AllocateStatus,'deltaimp in skkr' )
c     - - - - - - - - - - - - - - - - - - - - - -
        call pothandleimp(
     > lmax,nimp,v00,ivacpot,opot,imppot,dx,ns,rs,idpota,
...
c    -------------------------------------------------------------------
c      call restart_in(itscf0,etop(npanel),v0,conc,opot,
c     >                vra,bra,bopra,rba,vrb,brb,boprb,rbb,
c     >                bulk,vrsbulk,nintfc,idpota,idpotb,za,zb,
c     >                laycore,printout)
c     end if
c    -------------------------------------------------------------------
      write(6,*)
...
c     ------------------------------------------------------------------
     ...
     > tau_kint,taua_kint,taub_kint,dbogomaxp)
      call cpu_time(tfinish)
      write(6,'('' Time ellapsed in tcpa_in:'',t35,f8.1,'' s'')')
     > tfinish-tstart
c      if (cpamatin) then
...
c     ------------------------------------------------------------------
     ...
     &             mkpar)
      call cpu_time(tfinish)
      write(6,'('' Time ellapsed in kmesh2d:'',t35,f8.1,'' s'')')
     > tfinish-tstart
c     ------------------------------------------------------------------ 
c
      if(itest.ge.1) then
...
c  --------------- cluster -----------------
     >   kpair,npair,tau_ii1,tau_ij1,tau_ji1,taupath,
     >   clucalc,hostprint)
c
      else
...
c          stop 'after tcpa_out'
       ...
       call cpu_time(tfinish)
       write(6,'('' Time ellapsed in tcpa_out:'',t35,f8.1,'' s'')')
     > tfinish-tstart
c
c
