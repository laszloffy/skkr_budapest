c     Name of Copyright owners: Gabor Csire, Andras Laszloffy, Bendeguz Nyari, Laszlo Szunyogh, and Balazs Ujfalussy
     ...
     >           qvpa,qvpb,qva,qvb,dosmaga,dosmagb,dosmagh,
...
c       ---------------------------------------
        call dosprint(lms,lmax,li,ne,cear,dosteh)
        call pdosprint(lms,lmax,li,ne,cear,dosteh)
c       ---------------------------------------
        if (cpalay) then
          write(7,'(''##'',i3,t10,''DOS triplet up'')') li
          write(8,'(''##'',i3,t10,a,''PDOS triplet up'')') li,idpotb(li)
...
c       ---------------------------------------
        call dosprint(lms,lmax,li,ne,cear,dosthe)
        call pdosprint(lms,lmax,li,ne,cear,dosthe)
c       ---------------------------------------
        if (cpalay) then
          write(7,'(''##'',i3,t10,''DOS triplet down'')') li
...
c
      do ie=1,ne
        ...
        if(.not.lms) then
          ...
          write(7,'(f10.6,4x,16d14.5)') er,(ddd(k),k=1,kmax),sum
        else
          ii=0
          do is=1,2
             sum=0.d0
             ...
             write(7,'(f10.6,4x,16d14.5)') er,(ppp(l,is),l=0,lmax),sum
          end do
        end if
      end do
c
...
c
      do ie=1,ne
        ...
        if(.not.lms) then
          ...
          write(8,'(f10.6,2x,15f10.5)') er,(dos(k,li,ie),k=1,kmymax),sum
        else
          ii=0
          do is=1,2
             sum=0.d0
             do l=0,lmax
                do m=-l,l
                   ii=ii+1
                   sum=sum+dos(ii,li,ie)
                   ppp(ii,is)=dos(ii,li,ie)
                end do
             end do
             write(8,'(f10.6,2x,15f10.5)') er,(ppp(l,is),l=0,lmmax),sum
          end do
        end if
      end do
c
