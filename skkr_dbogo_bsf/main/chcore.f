...
c
     ...
     >      write(6,'(''  routine CHCORE> '')')
c
      do 110 li=1,nintfc
        cpalay=1.d0-conc(li).gt.tiny
        if(itest.gt.2) then
        write(6,'(a10)') idpota(li)
        write(6,'(f10.5)') conc(li)
        write(6,'(f6.2,d20.10,i5,f15.10)') za(li),dx(li),ns(li),rs(li)
        write(6,'(4d20.10)') (vra(j,li),j=1,nrad)
        end if
c
     ...
     >      dfla(i,li),dq1a(i,li),dena(i,li)
          ...
          call rcore(idpota(li),za(li),vra(1,li),
...
c
     ...
     >      dflb(i,li),dq1b(i,li),denb(i,li)
          ...
          call rcore(idpotb(li),zb(li),vrb(1,li),
